...
> 所有可行候选位置产生一个四维元组 `(P1, P2, P3, P4)`，按字典序从小到大排序，最小者即为最优。

| 优先级 | 特征键 | 计算方式 | 优化方向 | 物理含义 |
|--------|--------|---------|---------|---------|
| **P1** | `void_volume` | $\sum \max(0, h_{place} - h_{region}) \times r^2$ | 越小越好 | 底部空隙体积，反映填充紧密度 |
...
> [!TIP]
> 字典序策略的核心优势：P1 严格优先于 P2，P2 严格优先于 P3 ... 不存在权重折中问题，物理意义清晰，无需人工调参。
...
```python
{
    ...
    'sort_key': (void_vol, -adj, z_max, corner_dist),  # 排序特征
    'orientation': {                       # 朝向信息
        'base_dims': (bx, by),
        'up_dim': float,
