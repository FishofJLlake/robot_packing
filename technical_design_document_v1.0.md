...
- **效率低下**：人工装箱速度难以匹配自动化分拣线的吞吐需求
- **空间浪费**：缺乏全局最优视角，装载率长期偏低（通常仅 30%-40%）
...
| 文件 | 类型 | 代码行数 | 职责 |
|------|------|---------|------|
| [config.py](file:///d:/Code/robot_packing/config.py) | 配置 | 194 | 全局超参数、朝向表、物理约束 |
| [point_cloud_processor.py](file:///d:/Code/robot_packing/point_cloud_processor.py) | 核心模块 | 591 | 点云预处理、高度图生成、坐标系变换 |
| [packing_planner.py](file:///d:/Code/robot_packing/packing_planner.py) | 核心模块 | 632 | 装箱规划核心算法 |
| [stability_checker.py](file:///d:/Code/robot_packing/stability_checker.py) | 核心模块 | 362 | 支撑面积、倾斜角、重心稳定性检测 |
| [pose_generator.py](file:///d:/Code/robot_packing/pose_generator.py) | 核心模块 | 114 | 6D位姿合成与格式化 |
| [mujoco_simulator.py](file:///d:/Code/robot_packing/mujoco_simulator.py) | 仿真 | 111 | 单物品倾覆物理模拟 |
| [visualize_mujoco.py](file:///d:/Code/robot_packing/visualize_mujoco.py) | 仿真/可视化 | 193 | 全序列物理回放 |
| [main.py](file:///d:/Code/robot_packing/main.py) | 入口 | 441 | 命令行解析与多模式演示 |
| [visualizer.py](file:///d:/Code/robot_packing/visualizer.py) | 可视化 | 307 | 高度图/俯视图/3D静态可视化 |
| [visualize_demo.py](file:///d:/Code/robot_packing/visualize_demo.py) | 可视化 | 556 | Matplotlib逐步3D动画 |
| [test_packing.py](file:///d:/Code/robot_packing/test_packing.py) | 测试 | 432 | 单元测试套件 |
...
在聚合后对有效区域施加 3×3 中值滤波（`_masked_median_filter`），仅对 `valid_mask=True` 的区域操作，避免无数据区域的干扰。
...
> 所有可行候选位置产生一个五维元组 `(P0, P1, P2, P3, P4)`，按字典序从小到大排序，最小者即为最优。
...
| **P1** | `void_volume` | $\sum \max(0, h_{place} - h_{region}) \times r^2$ | 越小越好 | 底部空隙体积，反映表面贴合程度 |
| **P2** | `-adjacency` | 四面贴合度取负 | 越小越好 | 与墙壁/已有物品的贴合程度 |
| **P3** | `z_max` | $h_{place} + h_{up}$ | 越小越好 | 放置后顶面高度，"先低后高" |
...
> [!TIP]
> 字典序策略的核心优势：P0 严格优先于 P1，P1 严格优先于 P2 ... 不存在权重折中问题，物理意义清晰，无需人工调参。P0 与 P1 测量维度正交：P0 是战略级的空间分配决策（"放哪个区域"），P1 是战术级的表面质量决策（"区域内哪个精确位置"）。
...
flowchart TD
    ...
    B -->|否| X["❌ 拒绝"]
    B -->|是| C{"稳定性检测<br/>支撑面积 ≥ 60%?<br/>倾斜角 ≤ 5°?<br/>重心在支撑凸包内?"}
    C -->|不稳定| D{"Plus策略启用?"}
    D -->|否| X
    D -->|是| E{"MuJoCo物理仿真<br/>模拟倾覆后是否仍在笼内?"}
    E -->|滑出笼外| X
    E -->|留在笼内| F["✅ 候选通过（带仿真位姿）"]
    C -->|稳定| G{"外侧高度约束<br/>外侧不超过里侧?"}
    G -->|违规率过高| X
    G -->|通过| H["✅ 候选通过"]
```
...
随着填充率增加，系统动态放宽**外侧高度约束**（但支撑面积约束始终严格）：
...
> [!WARNING]
> 支撑面积比（MIN_SUPPORT_RATIO = 60%）**永不松弛**。这是物理安全约束，松弛可能导致货物倾倒出笼。
...
```python
support_mask = (place_height - region) <= SUPPORT_HEIGHT_TOLERANCE  # 5cm容差
...
4. **有效区域中值滤波**：创新性地使用 `NaN标记 + nanmedian` 方法，避免无数据区域对有效数据的稀释
...
```
check_stability()
├── 支撑面积比检测 → support_ratio ≥ 60%
├── 底面倾斜角检测 → tilt_angle ≤ 5°
...
sequenceDiagram
    ...
    loop 每种朝向
        ...
        loop 每个候选位置
            ...
            SC-->>PP: {is_stable, support_ratio, tilt_angle, ...}
            PP->>PP: 外侧高度约束检查
            PP->>PP: 计算字典序特征元组
        end
    end
...
```python
{
    ...
    'sort_key': (-fit_ratio, void_vol, -adj, z_max, corner_dist),  # 排序特征（5元组）
    'orientation': {                       # 朝向信息
        'base_dims': (bx, by),
        'up_dim': float,
...
所有超参数集中于 [config.py](file:///d:/Code/robot_packing/config.py)，分为以下组：
...
| `MIN_SUPPORT_LENGTH_RATIO_Y` | 0.60 | Plus策略下里侧支撑深度比最小值 |
...
| 参数 | 默认值 | 描述 |
|------|--------|------|
| `PLY_CROP_X` | (-0.92, -0.02) | X方向裁剪范围 |
| `PLY_CROP_Y` | (0.0, 0.60) | Y方向裁剪范围 |
| `PLY_CROP_Z` | (-1.2, 0.50) | Z方向裁剪范围 |
...
测试文件 [test_packing.py](file:///d:/Code/robot_packing/test_packing.py) 包含 10 个测试用例：
...
| 4 | `test_stability_checker` | `StabilityChecker.check_stability` | 平坦底面/完全支撑/稀疏支撑/严重倾斜四种场景 |
| 5 | `test_pose_generation` | `compute_6d_pose` | 位置计算精度、四元数单位化、变换矩阵正确性 |
| 6 | `test_packing_planner_empty_cage` | `PackingPlanner.plan_placement` | 空笼首物放置于里-左-底 |
| 7 | `test_packing_xy_only` | `PackingPlanner(xy_only=True)` | XY-only模式下 roll/pitch 始终为0 |
| 8 | `test_outer_height_constraint` | `PackingPlanner._check_outer_height_constraint` | 外侧物品不遮挡里侧取件路径 |
| 9 | `test_packing_sequence` | 连续4件装箱 | 多物品连续放置、统计信息正确 |
| 10 | `test_oversized_item` | 超大物品 (2m³) | 超出笼体的物品返回 None |
...
| **稳定性通过率** | 所有放置的支撑率均 ≥ 60% | 100% |
| **物理仿真一致性** | MuJoCo回放中无物品飞出笼外 | 100% |
