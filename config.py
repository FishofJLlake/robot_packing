...
OUTER_HEIGHT_TOLERANCE = 0.05       # 5cm
...
SUPPORT_HEIGHT_TOLERANCE = 0.01  # 支撑高度容差（1cm以内视为有支撑）

# ============================================================
# 评分权重（体现先里后外、先左后右、先下后上）
# ============================================================
WEIGHT_DEPTH = 15.0    # Y方向（越里越好，越大分越高）
WEIGHT_LEFT  = 5.0     # X方向（越左越好）
WEIGHT_LOW   = 10.0    # Z方向（越低越好，提高权重强化从下往上）
WEIGHT_FIT   = 3.0     # 紧凑度奖励（与已有货物紧贴）
WEIGHT_SURFACE_FLAT = 4.0    # 放置后表面平整度（越平越好）
WEIGHT_LAYER_COMPLETE = 3.0  # 层完成度（同层高度一致加分）
WEIGHT_VOID_PENALTY = -5.0   # 空隙惩罚（底部空隙越大扣分越多）
WEIGHT_STEP_PROFILE = 4.0    # 阶梯轮廓奖励（里高外低加分）
